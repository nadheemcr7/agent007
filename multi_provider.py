...
class MultiProvider(ModelProvider):
    ...
    - "openai/" prefix or no prefix -> OpenAIProvider. e.g. "openai/gpt-4.1", "gpt-4.1"
    - "litellm/" prefix -> LitellmProvider. e.g. "litellm/openai/gpt-4.1"
    - "groq/" prefix -> GroqProvider. e.g. "groq/llama3-8b-8192"
        ...
        *,
        ...
        Args:
            provider_map: A MultiProviderMap that maps prefixes to ModelProviders. If not provided,
                we will use a default mapping. See the documentation for this class to see the
                default mapping.
            openai_api_key: The API key to use for the OpenAI provider.
            openai_base_url: The base URL to use for the OpenAI provider.
            openai_client: An optional OpenAI client to use.
            openai_organization: The organization to use for the OpenAI provider.
            openai_project: The project to use for the OpenAI provider.
            openai_use_responses: Whether to use the OpenAI responses API.
            groq_api_key: The API key to use for the Groq provider.
            groq_base_url: The base URL to use for the Groq provider.
    ...
    def _create_fallback_provider(self, prefix: str) -> ModelProvider:
        print(f"DEBUG: _create_fallback_provider called with prefix: '{prefix}' (length: {len(prefix)})")
        print(f"DEBUG: Type of prefix: {type(prefix)}")
        if prefix == "litellm":
            print("DEBUG: Prefix is 'litellm'")
            from ..extensions.models.litellm_provider import LitellmProvider
            return LitellmProvider()
        elif prefix == "groq":
            print("DEBUG: Prefix is 'groq', returning self.groq_provider")
            return self.groq_provider
        else:
            print(f"DEBUG: Prefix '{prefix}' is unknown, raising UserError")
            raise UserError(f"Unknown prefix: {prefix}")
